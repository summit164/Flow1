...
async function clearWebhookAndRefresh() {
  try {
    ...
    await bot.setMyCommands([
      ...
      { command: 'clear_cart', description: 'Очистить корзину (локальную историю)' },
...
async function clearAllBotSettings() {
  try {
    ...
    await bot.setMyCommands([
      ...
      { command: 'myorders', description: 'Мои заказы' },
      { command: 'clear_cart', description: 'Очистить корзину (локальную историю)' },
      { command: 'debug', description: 'Диагностика проблем с заказами' },
      { command: 'clear', description: 'Очистить кэш' }
    ]);
...
// Хранилище для локальной истории заказов пользователей (не затрагивает БД)
const userLocalOrders = new Map();
...
// Обработчик команды /clear_cart
bot.onText(/\/clear_cart/, async (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  
  // Проверяем, есть ли у пользователя локальные данные для очистки
  const userPhone = userPhones.get(userId);
  const userLocalOrdersData = userLocalOrders.get(userId);
  
  if (!userPhone && !userLocalOrdersData) {
    bot.sendMessage(
      chatId,
      '🛒 У вас нет локальных данных для очистки.\n\n' +
      '💡 Эта команда очищает только локальную историю в боте, не затрагивая базу данных.\n\n' +
      'Для просмотра заказов используйте /myorders'
    );
    return;
  }
  ...
  bot.sendMessage(
    chatId,
    '🗑️ Вы уверены, что хотите очистить локальную корзину?\n\n' +
    '⚠️ Это действие:\n' +
    '• Удалит вашу локальную историю заказов в боте\n' +
    '• Очистит связь с номером телефона\n' +
    '• НЕ затронет заказы в базе данных\n\n' +
    '💡 После очистки вам нужно будет заново зарегистрировать номер телефона командой /phone',
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: '✅ Да, очистить', callback_data: 'confirm_clear_cart' },
            { text: '❌ Отмена', callback_data: 'cancel_clear_cart' }
          ]
        ]
      }
    }
...
// Обработчик callback-кнопок для подтверждения очистки корзины
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (data === 'confirm_clear_cart') {
    ...
    userLocalOrders.delete(userId);
    
    // Отправляем подтверждение
    await bot.editMessageText(
      '✅ Локальная корзина успешно очищена!\n\n' +
      '🧹 Удалено:\n' +
      '• Связь с номером телефона\n' +
      '• Локальная история заказов\n\n' +
      '💡 Заказы в базе данных остались нетронутыми.\n' +
      'Для повторной связи с заказами используйте команду /phone',
      {
        chat_id: chatId,
    ...
    console.log(`🗑️ Пользователь ${userId} очистил локальную корзину`);
    
  } else if (data === 'cancel_clear_cart') {
    // Отменяем операцию
    await bot.editMessageText(
      '❌ Очистка корзины отменена.\n\n' +
      '🛒 Ваши локальные данные остались без изменений.',
      {
        chat_id: chatId,
        message_id: callbackQuery.message.message_id,
        reply_markup: {
...
async function getUserOrders(userId) {
  try {
    ...
    console.error(`❌ Ошибка при получении заказов для ${userId}:`, error.message);
    if (error.response) {
...
// Диагностическая команда для проверки связи с заказами
bot.onText(/\/debug/, async (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  
  try {
    let debugMessage = '🔍 **ДИАГНОСТИКА СВЯЗИ С ЗАКАЗАМИ**\n\n';
    
    // Проверяем локальные данные пользователя
    const userPhone = userPhones.get(userId);
    debugMessage += `👤 **Ваши данные в боте:**\n`;
    debugMessage += `• Telegram ID: ${userId}\n`;
    
    if (userPhone && userPhone.phone) {
      debugMessage += `• Зарегистрированный телефон: +${userPhone.phone}\n`;
    } else {
      debugMessage += `• Телефон: ❌ не зарегистрирован\n`;
    }
    
    debugMessage += `\n🔍 **Поиск заказов:**\n`;
    
    // Проверяем заказы по Telegram ID
    const telegramOrders = await getUserOrders(userId.toString());
    debugMessage += `• По Telegram ID (${userId}): ${telegramOrders ? telegramOrders.length : 0} заказов\n`;
    
    // Проверяем заказы по телефону, если он есть
    if (userPhone && userPhone.phone) {
      const normalizedPhoneId = `phone_${userPhone.phone.replace(/\D/g, '')}`;
      const phoneOrders = await getUserOrders(normalizedPhoneId);
      debugMessage += `• По телефону (${normalizedPhoneId}): ${phoneOrders ? phoneOrders.length : 0} заказов\n`;
      
      // Проверяем альтернативный формат телефона
      const originalPhone = userPhone.phone.replace(/\D/g, '');
      const altPhoneId = originalPhone.startsWith('7') ? 
        `phone_8${originalPhone.slice(1)}` : `phone_${originalPhone}`;
      const altPhoneOrders = await getUserOrders(altPhoneId);
      debugMessage += `• По телефону альт. (${altPhoneId}): ${altPhoneOrders ? altPhoneOrders.length : 0} заказов\n`;
    }
    
    debugMessage += `\n💡 **Рекомендации:**\n`;
    
    if (!userPhone || !userPhone.phone) {
      debugMessage += `• Зарегистрируйте телефон командой /phone\n`;
    }
    
    debugMessage += `• При оформлении заказа на сайте обязательно укажите:\n`;
    debugMessage += `  - Telegram ID: ${userId}\n`;
    if (userPhone && userPhone.phone) {
      debugMessage += `  - Или телефон: +${userPhone.phone}\n`;
    }
    
    debugMessage += `\n🔧 **Если проблема не решается:**\n`;
    debugMessage += `• Попробуйте команду /clear_cart и заново зарегистрируйте телефон\n`;
    debugMessage += `• Убедитесь, что при заказе на сайте указываете правильные данные\n`;
    debugMessage += `• Свяжитесь с поддержкой: @RyazanovKirill`;
    ...
    bot.sendMessage(
      chatId,
      '❌ Произошла ошибка при выполнении диагностики. Попробуйте позже.'
    );
  }
});
  ...
  try {
    ...
    if (orders.length === 0) {
      ...
      if (!userPhone || !userPhone.phone) {
        ...
        message += '• Заказы не были связаны с вашим аккаунтом\n';
        message += '• Вы еще не делали заказов\n\n';
        message += '💡 **Для связи заказов с ботом:**\n';
        message += '• При оформлении заказа на сайте укажите ваш Telegram ID: `' + userId + '`\n';
        message += '• Или зарегистрируйте номер телефона командой /phone\n\n';
        message += '🔧 **Если заказы есть, но не отображаются:**\n';
        message += '• Используйте команду /debug для диагностики\n\n';
        message += '🌺 Перейдите в каталог, чтобы выбрать букеты и цветы.';
      } else {
        message += '🔍 **Возможные причины:**\n';
        message += '• При оформлении заказа не был указан ваш Telegram ID или телефон\n';
        message += '• Заказы еще обрабатываются\n\n';
        message += '💡 **При оформлении заказа на сайте укажите:**\n';
        message += '• Telegram ID: `' + userId + '`\n';
        message += '• Или номер телефона: `+' + userPhone.phone + '`\n\n';
        message += '🔧 **Если заказы есть, но не отображаются:**\n';
